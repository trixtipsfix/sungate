...
<?php
    $kelime = g('kelime');
    $sorgu = $db->get_var("SELECT COUNT(*) FROM the_hotel WHERE name LIKE '%$kelime%' AND status = 1");
?>
            ...
            <div class="col-lg-12">
                <div class="isotope-wrapper">
                    <div class="row">
                        ...
                        $oteller = $db->get_results("SELECT * FROM the_hotel WHERE name LIKE '%$kelime%' LIMIT $kacar OFFSET $nereden");
...
<?php }else{ ?>
    ...
    <div class="container">
        <div class="row">
            ...
            <?php
            @$sayfa = $_GET["sayfa"];
            $oteller = $db->get_results("SELECT * FROM the_hotel ORDER BY rand() LIMIT 6");
            foreach ($oteller as $otel){
                if($otel->picture) {
                    $min_cover = 'data/hotel/' . $otel->picture;
                }else{
